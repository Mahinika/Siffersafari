...
void main() {
  ...
  testWidgets(
    'Integration (QA, opt-in RUN_TAP_COUNT=1): mät antal tryck i huvudflöden',
    (tester) async {
      ...
      Future<void> tap(
        ...
        await it.tap(tester, finder, after: settle);
        taps++;
      }
      ...
      Future<void> ensureHomeByVersionText() async {
        ...
        fail(
          'Did not return to Home. Visible texts: ${it.visibleTexts(tester).take(80).toList()}',
        );
      }
      ...
      Future<void> startFirstAvailableQuiz() async {
        ...
        if (chosenOperation == null) {
          fail(
            'No operation cards on Home. Visible texts: ${it.visibleTexts(tester).take(80).toList()}',
          );
        }
        ...
        for (var i = 0; i < 20; i++) {
          if (find.byType(AnswerButton).evaluate().isEmpty) {
            ...
            if (find.byType(Dialog).evaluate().isNotEmpty &&
              ...
              if (find.text('Tillbaka till Start').evaluate().isNotEmpty) {
                return;
              }
              continue;
            }
      ...
      if (createUserHomeButton.evaluate().isNotEmpty) {
        ...
        expect(
          ...
          isTrue,
          reason:
              'Expected ops onboarding step. Visible texts: ${it.visibleTexts(tester).take(40).toList()}',
        );
        ...
        final practiceButton =
            find.widgetWithText(ElevatedButton, practiceLabel);
    ...
    skip: Platform.environment['RUN_TAP_COUNT'] != '1',
  );
}
